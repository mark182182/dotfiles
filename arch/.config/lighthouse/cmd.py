...
def find(query):
  ...
  for i in xrange(min(5, len(find_array))):
    append_output(str(find_array[i]),"termite -e bash -c 'if [[ $(file "+find_array[i]+" | grep text) != \"\" ]]; then vim "+find_array[i]+"; else cd $(dirname "+find_array[i]+"); bash; fi;'");
  update_output()
        ...
        try:
            ...
            if (type(out) != str and str(out)[0] == '<'):
                ...
                prepend_output("python: "+str(out),
                            "termite -e python2.7 -i -c 'print "+userInput+"'")
